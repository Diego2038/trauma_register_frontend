...
class TraumaRegisterIcd10Content extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return ExpandableTitleWidget(
      ...
      index: 8,
      expandedWidget:
          _getCurrentPatientData(context).traumaRegisterIcd10 == null ||
                  _getCurrentPatientData(context).traumaRegisterIcd10!.isEmpty
              ? allowChanges
  ...
  Widget _addNewElement(BuildContext context) {
    return CustomIconButton(
      onPressed: () {
        ...
        final currentElements = currentPatientData.traumaRegisterIcd10;
...
class _Content extends StatefulWidget {
  ...
  final int keyy;
  final TraumaRegisterIcd10 value;
  final CustomSize customSize;
  final ActionType action;
  final bool freeSize;
...
class _ContentState extends State<_Content> {
    ...
    return CustomContainer(
      ...
      onUpdate: () async {
        final bool isANewElement = _getCurrentPatientData(context)
                .traumaRegisterIcd10![widget.keyy]
                .id ==
            null;
        ...
        final element =
            _getCurrentPatientData(context).traumaRegisterIcd10![widget.keyy];
        ...
        final result = await (isANewElement
            ? _getCurrentProvider(context)
                .createTraumaRegisterIcd10(element, id)
            : _getCurrentProvider(context).updateTraumaRegisterIcd10(element));
        if (isANewElement) {
          _updateElements(
            context: context,
            id: result.idElement,
      ...
      onDelete: () async {
        ...
        if (widget.action == ActionType.actualizar && !isANewElement) {
          ...
          final result = await _getCurrentProvider(context)
              .deleteTraumaRegisterIcd10ById(_getCurrentPatientData(context)
                  .traumaRegisterIcd10![widget.keyy]
                  .id
                  .toString());
          CustomModal.showModal(
        ...
        _getCurrentProvider(context).updatePatientData(
          _getCurrentPatientData(context).copyWith(
            traumaRegisterIcd10: _getCurrentPatientData(context)
                .traumaRegisterIcd10
                ?.asMap()
                .entries
      ...
      children: traumaRegisterIcd10Content(
        context: context,
        index: widget.keyy,
        traumaRegisterIcd10: widget.value,
  ...
  List<Widget> traumaRegisterIcd10Content({
    ...
    return [
      CustomInputWithLabel(
        ...
        onChanged: (String? value) {
          ...
          traumaDataProvider.updatePatientData(patientData.copyWith(
            traumaRegisterIcd10: patientData.traumaRegisterIcd10
                ?.asMap()
                .entries
                .map((e) => e.key == index
                    ? e.value.copyWith(
      ...
      CustomInputWithLabel(
        ...
        lines: 1,
        width: freeSize ? null : 460,
        suggestions: ContentOptions.traumaRegisterICD10.mecanismoICD,
        inputType: InputType.string,
        onChanged: (String? value) {
          final patientData = _getCurrentPatientData(context);
          traumaDataProvider.updatePatientData(patientData.copyWith(
            traumaRegisterIcd10: patientData.traumaRegisterIcd10
                ...
                .map((e) => e.key == index
                    ? e.value.copyWith(
                        mecanismoIcd: Optional<String?>.of(
                            TransformData.getTransformedValue<String>(value)))
                    : e.value)
                .toList(),
          ));
  ...
  void _updateElements({
    ...
    final traumaDataProvider = _getCurrentProvider(context);
    final patientData = _getCurrentPatientData(context);
    final elements = patientData.traumaRegisterIcd10;
    if (elements == null) return;
    elements[index] = elements[index].copyWith(id: id);
    traumaDataProvider.updatePatientData(patientData.copyWith(
      traumaRegisterIcd10: elements,
    ));
  }
}
