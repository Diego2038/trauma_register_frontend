...
class _ContentState extends State<_Content> {
    ...
    return CustomContainer(
      ...
      onUpdate: () async {
        final bool isANewElement =
            _getCurrentPatientData(context).poisoningInjury![widget.keyy].id ==
                null;
        ...
        final element =
            _getCurrentPatientData(context).poisoningInjury![widget.keyy];
        final id = _getCurrentPatientData(context).traumaRegisterRecordId!;
        final result = await (isANewElement
            ? _getCurrentProvider(context).createPoisoningInjury(element, id)
            : _getCurrentProvider(context).updatePoisoningInjury(element));
        if (isANewElement) {
          _updateElements(
            context: context,
            id: result.idElement,
      ...
      onDelete: () async {
        ...
        if (widget.action == ActionType.actualizar && !isANewElement) {
          ...
          final result = await _getCurrentProvider(context)
              .deletePoisoningInjuryById(_getCurrentPatientData(context)
                  .poisoningInjury![widget.keyy]
                  .id
                  .toString());
          NavigationService.pop();
          CustomModal.showModal(
        ...
        _getCurrentProvider(context).updatePatientData(
          _getCurrentPatientData(context).copyWith(
            poisoningInjury: _getCurrentPatientData(context)
                .poisoningInjury
                ?.asMap()
                .entries
  ...
  List<Widget> poisoningInjuryContent({
    ...
    return [
      CustomInputWithLabel(
        ...
        suggestions: ContentOptions.poisonInjury.tipoDeEnvenenamiento,
        ...
        onChanged: (String? value) {
          ...
          traumaDataProvider.updatePatientData(patientData.copyWith(
            poisoningInjury: patientData.poisoningInjury
                ...
                .map((e) => e.key == index
                    ? e.value.copyWith(
                        tipoDeEnvenenamiento: Optional<String?>.of(
  ...
  void _updateElements({
    ...
    final traumaDataProvider = _getCurrentProvider(context);
    final patientData = _getCurrentPatientData(context);
    final elements = patientData.poisoningInjury;
    if (elements == null) return;
    elements[index] = elements[index].copyWith(id: id);
    traumaDataProvider.updatePatientData(patientData.copyWith(
      poisoningInjury: elements,
    ));
  }
}
