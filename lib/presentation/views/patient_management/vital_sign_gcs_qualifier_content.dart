...
class _Content extends StatefulWidget {
  ...
  final int keyy;
  final VitalSignGcsQualifier value;
  final CustomSize customSize;
  final ActionType action;
  final bool freeSize;
...
class _ContentState extends State<_Content> {
    ...
    return CustomContainer(
      ...
      onUpdate: () async {
        final bool isANewElement = _getCurrentPatientData(context)
                .vitalSignGcsQualifier![widget.keyy]
                .id ==
            null;
        ...
        final element =
            _getCurrentPatientData(context).vitalSignGcsQualifier![widget.keyy];
        ...
        final result = await (isANewElement
            ? _getCurrentProvider(context)
                .createVitalSignGcsQualifier(element, id)
            : _getCurrentProvider(context)
                .updateVitalSignGcsQualifier(element));
        if (isANewElement) {
          _updateElements(
            context: context,
            id: result.idElement,
      ...
      onDelete: () async {
        ...
        if (widget.action == ActionType.actualizar && !isANewElement) {
          ...
          final result = await _getCurrentProvider(context)
              .deleteVitalSignGcsQualifierById(_getCurrentPatientData(context)
                  .vitalSignGcsQualifier![widget.keyy]
                  .id
                  .toString());
          NavigationService.pop();
        ...
        _getCurrentProvider(context).updatePatientData(
          _getCurrentPatientData(context).copyWith(
            vitalSignGcsQualifier: _getCurrentPatientData(context)
                .vitalSignGcsQualifier
                ?.asMap()
                .entries
      ...
      children: vitalSignGcsQualifierContent(
        context: context,
        index: widget.keyy,
        vitalSignGcsQualifier: widget.value,
  ...
  List<Widget> vitalSignGcsQualifierContent({
    ...
    return [
      CustomInputWithLabel(
        ...
        suggestions: ContentOptions.vitalSignGCSQualifier.calificadorGCS,
        ...
        onChanged: (String? value) {
          ...
          traumaDataProvider.updatePatientData(patientData.copyWith(
            vitalSignGcsQualifier: patientData.vitalSignGcsQualifier
                ...
                .map((e) => e.key == index
                    ? e.value.copyWith(
                        calificadorGcs: Optional<String?>.of(
  ...
  void _updateElements({
    ...
    final traumaDataProvider = _getCurrentProvider(context);
    final patientData = _getCurrentPatientData(context);
    final elements = patientData.vitalSignGcsQualifier;
    if (elements == null) return;
    elements[index] = elements[index].copyWith(id: id);
    traumaDataProvider.updatePatientData(patientData.copyWith(
      vitalSignGcsQualifier: elements,
    ));
  }
}
