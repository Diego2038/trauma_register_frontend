...
class _ContentState extends State<_Content> {
    ...
    return CustomContainer(
      ...
      onUpdate: () async {
        final bool isANewElement =
            _getCurrentPatientData(context).device![widget.keyy].id == null;
        ...
        final element = _getCurrentPatientData(context).device![widget.keyy];
        final id = _getCurrentPatientData(context).traumaRegisterRecordId!;
        final result = await (isANewElement
            ? _getCurrentProvider(context).createDevice(element, id)
            : _getCurrentProvider(context).updateDevice(element));
        if (isANewElement) {
          _updateElements(
            context: context,
            id: result.idElement,
      ...
      onDelete: () async {
        ...
        if (widget.action == ActionType.actualizar && !isANewElement) {
          ...
          CustomModal.loadModal(context: context);
          final result = await _getCurrentProvider(context).deleteDeviceById(
              _getCurrentPatientData(context)
                  .device![widget.keyy]
                  .id
                  .toString());
        ...
        _getCurrentProvider(context).updatePatientData(
          _getCurrentPatientData(context).copyWith(
            device: _getCurrentPatientData(context)
                .device
                ?.asMap()
                .entries
  ...
  List<Widget> deviceContent({
    ...
    return [
      CustomInputWithLabel(
        ...
        suggestions: ContentOptions.device.tipoDeDispositivo,
  ...
  void _updateElements({
    ...
    final traumaDataProvider = _getCurrentProvider(context);
    final patientData = _getCurrentPatientData(context);
    final elements = patientData.device;
